...
namespace QuizoPlugins {
	...
	sealed class Localizer : LocalizedStringProvider
	{
		...
		public override string Name
		{
			get
			{
                if (fZh) return "鼠标停顿激活标签";
				return fJa ? "マウスホバーでタブを有効にします" : "Activate By MouseHover";
			}
		}
		...
		public override string Description
		{
			get
			{
                if (fZh) return "鼠标悬停可激活标签。要设置延迟时间，请按 Option 键。";
				return fJa ? "マウスホバーでタブを有効にします。遅延時間を設定するには、Option キーを押してください。" : "Activate a tab by mouse-hover. To set delay time, press Option. ";
			}
		}
